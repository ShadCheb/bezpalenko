<template>
    <div class="body__mask">
        <main class="p_main">
            <div class="p_main__header">
                <div class="p_main__caption">
                    <h1 class="caption__h2">Преподаватели</h1>
                </div>
            </div>
            <div class="container container--small">
                <div class="p_main__content">
                    <div class="p_main__img">
                        <div class="p_main__block-2">
                            <img src="../../public/images/prepodavateli/main-1.png" alt="">
                            <div class="p_main__img__name">Виктория Безпаленко</div>
                            <p class="p-text__16 p-text--upercase">Основатель школы,<br> руководитель 
                                и ведущий<br> преподаватель.</p>
                        </div>                        
                    </div>
                    <div class="p_main__text">
                        <div class="p_main__label">Факты</div>
                        <div class="p_main__block-1">
                            <ul class="process__ul">
                                <li>Эксперт образовательных знаний в Lash индустрии. Её мастер-классы 
                                    были проведены по всей России, а также в Италии, Австрии, Казахстане, 
                                    Польше, Латинской Америке, Испании, Турции, Молдове, во Вьетнаме, 
                                    Израиле и Париже</li>
                                <li>Действующий судья международных и Европейских Чемпионатов по 
                                    наращиванию ресниц</li>
                                <li>Победитель премии Лидер Года в номинации "Лучшая сеть студий 
                                    по Наращиванию ресниц в Крыму"</li>
                                <li>Лауреат Государственной премии "Сделано в России"</li>
                                <li>Обладатель Диплома высшего достоинства "За высокие показатели 
                                    качества профессиональной деятельности"</li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </main>

        <section class="p_quote">
            <div class="container">
                <article class="p_quote__text">
                    <p class="p-text__18 p-text--shift">Однажды, я обещала себе, что ученики моей школы 
                        будут получать самые современные и качественные знания. И вот уже 8 лет я держу 
                        своё обещание.</p>

                    <p class="p-text__18 p-text--shift">Я регулярно путешествую по странам СНГ и 
                        Европы с авторскими мастер-классами, чтобы привозить в Крым передовые 
                        техники и фишки для своих учениц.</p>
                </article>
            </div>
            <div class="p_quote__bcg">
            </div>
        </section>

        <section class="p_info">
            <div class="container">
                <article class="p_info__content">
                    <div class="p_info__video">
                        <div class="review__video-block" 
                          v-for="(value, name, index) in videoList"
                          :key="index"
                        >
                            <div class="video-block__video" :id="value.idContainer"></div>
                            <button type="button" 
                              class="video-block__play"
                              :data-video="name"
                              @click="clickPlayVideo($event)"
                            ></button>
                        </div>
                    </div>
                    <div class="p_info__text">
                        <div class="bcg__zag">
                        </div>
                        <p class="p-text__16">
                            В LashSchool V.Bezpalenko мы хотим дать нашим ученикам самую 
                            полезную и актуальную информацию, чтобы ты смогла приступить 
                            к работе уже на следующий день после обучения. Поэтому наши 
                            преподаватели - не теоретики, а практикующие мастера, которые 
                            постоянно продолжают обучаться новым техникам наращивания 
                            ресниц и оформления бровей
                        </p>
                        <div class="bcg__zag"></div>
                    </div>
                </article>
            </div>
        </section>

        <section class="p_we">
            <div class="container">
                <div class="p_we__blocks">
                    <article class="p_we__block">
                        <div class="p_we__img">
                            <img src="../../public/images/prepodavateli/we-1.jpg" alt="">
                        </div>
                        <p class="p-text__16">“К каждому ученику мы находим свой подход 
                            и не отойдём от тебя, пока ты не научишься правильно выполнять 
                            процедуру.”
                        </p>
                    </article>
                    <article class="p_we__block">
                        <div class="p_we__img">
                            <img src="../../public/images/prepodavateli/we-2.jpg" alt="">
                        </div>
                        <p class="p-text__16">“Мы передаем своим студентам не теорию, которую 
                            можно найти в Интернете, а реальные практические навыки, которыми 
                            владеем сами. Поэтому полученная в нашей школе профессия так высоко 
                            ценится и оплачивается.”
                        </p>
                    </article>
                    <article class="p_we__block">
                        <div class="p_we__img">
                            <img src="../../public/images/prepodavateli/we-3.jpg" alt="">
                        </div>
                        <p class="p-text__16">“Мы не гордимся своими многочисленными наградами 
                            и победами в Lash-конкурсах и чемпионатах. Настоящая гордость 
                            для нас - успехи наших выпускниц!”
                        </p>
                    </article>
                </div>
            </div>
        </section>

        <section class="p_btn">
            <div class="container">
                <div class="lp_button__circle">
                    <span></span>
                    <span></span>
                    <span></span>
                </div>
                <a href="/pages/result" class="btn-7">
                    Смотреть результаты учеников
                    <span class="btn-icon">
                        <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                        viewBox="0 0 512 512" xml:space="preserve">
                            <path d="M405.228,20.608c-4.824,0-9.792,0.624-14.752,1.848l3.816,15.528c30.248-7.424,56.624,15.592,56.624,44.32
                                c0,67.288-76.624,116.88-108.568,126.216l4.488,15.36c40.576-11.864,120.088-67.264,120.096-141.576
                                C466.932,48.28,439.252,20.608,405.228,20.608z"/>
                            <path d="M61.06,82.304c0-28.728,26.368-51.776,56.624-44.312l3.832-15.536c-40.032-9.84-76.448,21.008-76.448,59.848
                                c0,74.32,79.52,129.72,120.08,141.576l4.488-15.36C137.684,199.176,61.06,149.592,61.06,82.304z"/>
                            <path d="M283.74,297.8h-55.488c-4.416,0-8,3.576-8,8v30.36c0,2.408,1.096,4.704,2.976,6.224c1.432,1.16,3.208,1.776,5.024,1.776
                                c0.56,0,1.128-0.064,1.688-0.184c16.48-3.56,35.64-3.56,52.112,0c2.368,0.496,4.832-0.08,6.712-1.592
                                c1.88-1.52,2.976-3.8,2.976-6.224V305.8C291.74,301.376,288.164,297.8,283.74,297.8z M275.74,326.624
                                c-12.816-1.768-26.672-1.776-39.488,0.008V313.8v-0.008h39.488V326.624z"/>
                            <path d="M363.972,466.448H148.02c-4.416,0-8,3.576-8,8V504c0,4.424,3.584,8,8,8h215.952c4.424,0,8-3.576,8-8v-29.552
                                C371.972,470.024,368.396,466.448,363.972,466.448z M355.972,496H156.02v-13.552h199.952V496z"/>
                            <path d="M350.124,353.736H161.868c-4.416,0-8,3.576-8,8v112.712c0,4.424,3.584,8,8,8h188.256c4.416,0,8-3.584,8-8V361.736
                                C358.124,357.312,354.548,353.736,350.124,353.736z M342.124,466.448H169.868v-96.712h172.256V466.448z"/>
                            <path d="M318.132,385.728H193.868c-4.416,0-8,3.576-8,8v48.712c0,4.424,3.584,8,8,8h124.264c4.416,0,8-3.584,8-8v-48.712
                                C326.132,389.304,322.556,385.728,318.132,385.728z M310.132,434.44H201.868v-32.712h108.264V434.44z"/>
                            <path d="M396.988,4.024C394.428,1.432,391.02,0,387.388,0H124.612c-3.616,0-7.024,1.424-9.584,4c-2.56,2.584-3.96,6-3.944,9.632
                                c0.056,5.448,0.272,10.808,0.52,16.912c0.2,4.928,0.48,9.808,0.808,14.664c3.992,59.248,17.192,113.384,38.184,156.56
                                c3.088,6.352,6.424,12.576,9.904,18.48c16.032,27.224,34.392,46.752,54.592,58.056c2.12,1.184,4.64,1.336,6.848,0.456
                                c1.856,0.064,3.952,0.064,6.304,0.064h61.936c0.904,0.336,1.856,0.504,2.808,0.504c1.344,0,2.688-0.336,3.912-1.016
                                c20.192-11.296,38.56-30.832,54.592-58.056c3.456-5.896,6.784-12.112,9.888-18.472c21.008-43.2,34.208-97.344,38.192-156.568
                                c0.32-4.856,0.6-9.72,0.8-14.688c0.232-5.592,0.456-11.192,0.512-16.856C400.932,10.04,399.548,6.616,396.988,4.024z
                                M384.388,29.888c-0.208,4.792-0.464,9.528-0.784,14.248c-3.84,57.192-16.504,109.28-36.616,150.64
                                c-2.912,5.984-6.04,11.832-9.288,17.36c-13.648,23.176-29.504,40.632-46.008,50.68h-63.44c0.016,0.008-5.376,0-7.944,0
                                c-16.512-10.056-32.368-27.504-46.008-50.688c-3.264-5.544-6.392-11.376-9.296-17.352c-20.104-41.344-32.76-93.432-36.616-150.64
                                c-0.312-4.712-0.584-9.456-0.808-14.968c-0.176-4.376-0.36-8.752-0.44-13.168h257.704
                                C384.764,20.664,384.572,25.272,384.388,29.888z"/>
                            <path d="M352.108,34.64c-0.128,2.512-0.272,4.88-0.432,7.224c-3.336,49.568-14.616,96.288-31.768,131.552
                                c-2.4,4.928-5.008,9.808-7.752,14.48c-9.48,16.096-20.104,28.592-30.712,36.144l9.28,13.04
                                c12.432-8.848,24.6-23.048,35.224-41.064c2.96-5.048,5.776-10.296,8.352-15.6c18.024-37.08,29.864-85.904,33.344-137.496
                                c0.16-2.44,0.304-4.864,0.448-7.448L352.108,34.64z"/>
                            <path d="M283.74,262.824h-55.848c-13.464,0-14.8,26.104-14.8,26.368c0,0.248,1.568,24.616,15.16,24.616h55.856
                                c13.232,0,14.8-24.368,14.8-24.616C298.908,288.928,297.572,262.824,283.74,262.824z M280.82,297.808h-49.648
                                c-1.04-2.624-2.056-6.984-2.088-8.608c0.024-1.944,1.184-7.288,2.328-10.376h49.168c1.152,3.088,2.304,8.432,2.328,10.36
                                C282.884,290.824,281.86,295.192,280.82,297.808z"/>
                            <path d="M285.428,328.336c-18.688-4.024-40.168-4.04-58.864,0c-26.504,5.736-42.328,18.224-42.328,33.392c0,4.424,3.584,8,8,8
                                h127.52c4.416,0,8-3.576,8-8C327.756,346.552,311.932,334.072,285.428,328.336z M205.884,353.728
                                c4.944-3.72,13.056-7.368,24.056-9.752c16.488-3.56,35.648-3.56,52.104,0c11.008,2.384,19.12,6.032,24.064,9.752H205.884z"/>
                    </svg>
                    </span>
                </a>
                <div class="lp_button__circle">
                    <span></span>
                    <span></span>
                    <span></span>
                </div>
            </div>
        </section>
    </div>
</template>

<script>
export default {
  name: 'Prepodavateli',
  data() {
      return {
        videoList: {
          video1: {
            idContainer: 'video-1',
            idVideo: 'RXa4niEyXTI',
            api: null,
            link: 'info-video'
          }
        },
        templateVideo: null,
      };
  },
  mounted() {
    this.init();
  },
  methods: {
      init() {
        this.loadPlayer();
      },

      // инициализация
      loadPlayer() {
        // let YT;

        if (typeof(YT) == 'undefined' || typeof(YT.Player) == 'undefined') {
            let scriptYT = document.createElement("script");
    
            scriptYT.src = "https://www.youtube.com/iframe_api";
            
            document.body.appendChild(scriptYT);

            window.onYouTubePlayerAPIReady = () => {
                this.onYouTubePlayer();
            };
        } else this.onYouTubePlayer();
      },

      onYouTubePlayer() {
        for (let video in this.videoList) {
            let idContent = this.videoList[video].idContainer;
            let idVideo = this.videoList[video].idVideo;

            this.runAPIReady(idContent, idVideo, video);
        }
      },

      runAPIReady(container, video, number) {
          let self = this;

          this.videoList[number].api = new window['YT'].Player(container, {
              videoId: video,
              playerVars: {
                  modestbranding: 1,
                  rel: 0,
              },
              events: {
                  'onPlay': self.clickPlayVideo
              }
          });
      },

      clickPlayVideo(e) {
        let target = e.target;
        let number = target.dataset.video;

        target.classList.add('hidden');
        target.parentElement.classList.add('play');

        this.videoList[number].api.playVideo();
      },
  }
}
</script>

